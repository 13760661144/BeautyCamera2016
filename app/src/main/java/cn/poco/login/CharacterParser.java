...
public class CharacterParser {
	...
	private static CharacterParser mCharacterParser = new CharacterParser();

	public static CharacterParser getInstance() {
		return mCharacterParser;
	}
	...
	/** * 词组解析 * * @param chs * @return */
	public ArrayList<String> getSelling2(String chs) {
		String key, value;
		mEachChineseSpell = new ArrayList<String>();
		for (int i = 0; i < chs.length(); i++) {
			key = chs.substring(i, i + 1);
			if (key.getBytes().length >= 2) {
				value = (String) convert(key);
			...
			mEachChineseSpell.add(value);
		}
		return mEachChineseSpell;
	}
