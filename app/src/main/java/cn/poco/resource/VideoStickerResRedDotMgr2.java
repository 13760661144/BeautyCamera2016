...
public class VideoStickerResRedDotMgr2 extends BaseResMgr<VideoStickerRedDotRes, SparseArray<VideoStickerRedDotRes>>
{
...
//    public static final String CLOUD_URL = "http://tw.adnonstop.com/beauty/app/api/app_source/biz/beta/api/public/index.php?r=Switch/RedDotList";
    ...
    public static final String SDCARD_PATH = DownloadMgr.getInstance().RESOURCE_RED_DOT_PATH + "/sticker_red_dot.xxxx";
    public static final String CLOUD_CACHE_STICKER_PATH = DownloadMgr.getInstance().RESOURCE_RED_DOT_PATH + "/sticker_red_dot_cache.xxxx";
    ...
    private static VideoStickerResRedDotMgr2 sInstance;

    private VideoStickerResRedDotMgr2()
    {
    }


    public synchronized static VideoStickerResRedDotMgr2 getInstance()
    {
        if (sInstance == null)
        {
            sInstance = new VideoStickerResRedDotMgr2();
        }
        return sInstance;
    }
    ...
    public SparseArray<VideoStickerRedDotRes> getAllResArr(Context context)
    {
        if (!mHasAllResArr || mAllResArr == null || mAllResArr.size() == 0)
        {
            ...
            SparseArray<VideoStickerRedDotRes> downloadArr = sync_ar_GetCloudCacheRes(context, null);
            SparseArray<VideoStickerRedDotRes> sdcardArr = sync_GetSdcardRes(context, null);

            if (downloadArr != null && downloadArr.size() > 0)
            {
                SparseArray<VideoStickerRedDotRes> temp_arr = new SparseArray<>();

                //网络res
                for (int i = 0, size = downloadArr.size(); i < size; i++)
                {
                    VideoStickerRedDotRes download_res = downloadArr.valueAt(i);
                ...
                if (sdcardArr != null && sdcardArr.size() > 0)
                {
                    ...
                    {
                        VideoStickerRedDotRes sdcard_res = sdcardArr.valueAt(i);
                        VideoStickerRedDotRes download_res = temp_arr.get(sdcard_res.m_id);
                ...
                mHasAllResArr = true;
                mAllResArr = temp_arr;
                //Log.d("bbb", "VideoStickerResRedDotMgr2 --> getAllResArr: " + (System.currentTimeMillis() - s));
            }
        }
    ...
    public boolean hasMarkFlag(Context context, int id)
    {
        ...
        if (allResArr != null)
        {
            VideoStickerRedDotRes res = allResArr.get(id);
    ...
    public void markResFlag(Context context, int id)
    {
        ...
        if (allResArr != null)
        {
            VideoStickerRedDotRes markRes = allResArr.get(id);
            SparseArray<VideoStickerRedDotRes> sdcard_arr = sync_GetSdcardRes(context, null);
            if (sdcard_arr != null && markRes != null)
            {
     ...
     * @param ids
     */
    public void markResFlag(Context context, int[] ids)
    {
    ...
    protected Object sync_raw_ReadCloudData(Context context, DataFilter filter)
    {
        ...
        try
        {
            ...
            data = MaterialResourceProtocol.Get(CLOUD_URL,
                    ...
                    MaterialResourceProtocol.GetReqParams(PageType.STICKER, MaterialResourceProtocol.IS_DEBUG, new ResourceGroup[]{ResourceGroup.DOWNLOAD}),
     ...
     */
    @Override
    protected SparseArray<VideoStickerRedDotRes> sync_DecodeCloudRes(Context context, DataFilter filter, Object data)
    {
        SparseArray<VideoStickerRedDotRes> out = null;
        try
        {
            JSONObject jsonObject = new JSONObject(new String((byte[]) data));
            JSONArray jsonArr = VideoStickerResMgr2.GetRetDataList(jsonObject);
            VideoStickerRedDotRes item;
            Object obj;
            out = new SparseArray<>();
    ...
    @Override
    public boolean CheckIntact(VideoStickerRedDotRes res)
    {
        return super.CheckIntact(res);
    }


    @Override
    public VideoStickerRedDotRes ReadResItem(JSONObject jsonObject, boolean isPath)
    {
        VideoStickerRedDotRes out = null;

        if (jsonObject != null)
        {
            try
            {
                out = new VideoStickerRedDotRes();
    ...
    {
        return CLOUD_CACHE_STICKER_PATH;
    }

    @Override
    public VideoStickerRedDotRes GetItem(SparseArray<VideoStickerRedDotRes> arr, int id)
    {
        if (arr != null)
        {
            return arr.get(id);
    ...
    @Override
    public int GetResArrSize(SparseArray<VideoStickerRedDotRes> arr)
    {
        return arr.size();
    }
    ...
    @Override
    public boolean ResArrAddItem(SparseArray<VideoStickerRedDotRes> arr, VideoStickerRedDotRes item)
    {
    ...
    @Override
    protected SparseArray<VideoStickerRedDotRes> sync_raw_GetLocalRes(Context context, DataFilter filter)
    {
        //TODO 内置素材
        return null;
    }
     ...
     */
    @Override
    protected void sync_raw_SaveSdcardRes(Context context, SparseArray<VideoStickerRedDotRes> arr)
    {
    ...
    {
        return EventID.RED_DOT_STICKER_CLOUD_OK;
    }
}
